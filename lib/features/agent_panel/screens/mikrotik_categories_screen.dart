...
class _MikrotikCategoriesScreenState extends State<MikrotikCategoriesScreen>
            ...
            Container(
              ...
              child: Column(children: [
                TabBar(
                  ...
                  tabs: const [
                    Tab(icon: Icon(Icons.dns, color: Colors.greenAccent), text: 'سيرفرات'),
          ...
          child: Directionality(
            ...
            child: SingleChildScrollView(
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                TextField(
                    decoration: const InputDecoration(labelText: 'إسم شبكتك', border: OutlineInputBorder(), prefixIcon: Icon(Icons.dns, color: Colors.blue)),
                    controller: TextEditingController(text: name), onChanged: (v) => name = v),
                const SizedBox(height: 12),
                TextField(
                    decoration: const InputDecoration(labelText: 'موقع الشبكة', border: OutlineInputBorder(), prefixIcon: Icon(Icons.location_on, color: Colors.orange)),
                    controller: TextEditingController(text: location), onChanged: (v) => location = v),
                const SizedBox(height: 12),
                ExpansionTile(iconColor: Colors.teal, collapsedIconColor: Colors.teal, title: const Text('تفاصيل الموقع (اختياري)'), children: [
                  TextField(decoration: const InputDecoration(labelText: 'المحافظة', border: OutlineInputBorder(), prefixIcon: Icon(Icons.map, color: Colors.teal)),
                      controller: TextEditingController(text: governorate), onChanged: (v) => governorate = v),
                  const SizedBox(height: 12),
                  TextField(decoration: const InputDecoration(labelText: 'المديرية', border: OutlineInputBorder(), prefixIcon: Icon(Icons.map_outlined, color: Colors.teal)),
                      controller: TextEditingController(text: district), onChanged: (v) => district = v),
                  const SizedBox(height: 12),
                  ...coverageAreas.asMap().entries.map((entry) {
                    ...
                    return Padding(
                      ...
                      child: Row(children: [
                        Expanded(
                          child: TextField(
                              decoration: InputDecoration(labelText: 'منطقة البث ${idx + 1}', border: const OutlineInputBorder(), prefixIcon: const Icon(Icons.wifi_find, color: Colors.teal)),
                              controller: TextEditingController(text: coverageAreas[idx]), onChanged: (v) => coverageAreas[idx] = v),
                        ),
                ...
                const SizedBox(height: 12),
                TextField(
                    decoration: const InputDecoration(labelText: 'عنوان IP / الرابط (DDNS)', border: OutlineInputBorder(), prefixIcon: Icon(Icons.wifi, color: Colors.green)),
                    controller: TextEditingController(text: ip), onChanged: (v) => ip = v),
                const SizedBox(height: 12),
                TextField(
                    decoration: const InputDecoration(labelText: 'رابط صفحة تسجيل الدخول للزبائن', border: OutlineInputBorder(), prefixIcon: Icon(Icons.link, color: Colors.indigo)),
                    controller: TextEditingController(text: loginUrl), onChanged: (v) => loginUrl = v),
                const SizedBox(height: 12),
                Row(children: [
                  Expanded(child: TextField(decoration: const InputDecoration(labelText: 'مستخدم API', border: OutlineInputBorder(), prefixIcon: Icon(Icons.person, color: Colors.deepPurple)),
                      controller: TextEditingController(text: user), onChanged: (v) => user = v)),
                  const SizedBox(width: 10),
                  Expanded(child: TextField(decoration: const InputDecoration(labelText: 'كلمة المرور', border: OutlineInputBorder(), prefixIcon: Icon(Icons.lock, color: Colors.red)),
                      controller: TextEditingController(text: pass), obscureText: true, onChanged: (v) => pass = v)),
                ]),
                const SizedBox(height: 12),
                TextField(
                    decoration: const InputDecoration(labelText: 'API Port (الافتراضي: 8728)', border: OutlineInputBorder(), prefixIcon: Icon(Icons.settings_ethernet, color: Colors.blueGrey)),
                    controller: TextEditingController(text: port), onChanged: (v) => port = v),
                  ...
                  child: ElevatedButton(
                    ...
                    onPressed: isSubmitting ? null : () async {
                      if (name.isNotEmpty && location.isNotEmpty && ip.isNotEmpty) {
                        if (docId != null) { bool confirm = await _confirmAction("حفظ التعديلات", "هل أنت متأكد من حفظ التعديلات؟", Colors.blue); if (!confirm) return; }
                        _play('click');
                        ...
                        try {
                          ...
                          if (docId == null) {
                            networkData['status'] = 'متصل نشط 🟢'; networkData['categories'] = []; networkData['createdAt'] = FieldValue.serverTimestamp();
                          ...
                          _play('success');
                          if (mounted) { Navigator.pop(ctx); _showOverlayToast(docId == null ? 'تمت إضافة الشبكة بنجاح! 🟢' : 'تم التعديل بنجاح! ✏️'); }
                        } catch (e) { setModalState(() => isSubmitting = false); _play('error'); _showOverlayToast('فشل حفظ البيانات', isError: true); }
                      } else { _play('error'); _showOverlayToast('يرجى تعبئة الحقول الأساسية', isError: true); }
                    },
                ...
                SizedBox(
                  ...
                  child: OutlinedButton.icon(
                    ...
                    onPressed: () {
                      ...
                      _play('click'); Navigator.pop(ctx); _showOverlayToast('تم حفظ البيانات كمسودة');
                    },
  ...
  void _showAddCategoryBottomSheet(List<QueryDocumentSnapshot> agentNetworks,
    ...
    if (agentNetworks.isEmpty) { _play('error'); _showOverlayToast('يجب إضافة سيرفر شبكة أولاً!', isError: true); return; }
          ...
          child: Directionality(
            ...
            child: SingleChildScrollView(
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                Row(children: [
                  ...
                  ElevatedButton.icon(
                    onPressed: () async {
                      ...
                      if (pickedFile != null) { final bytes = await pickedFile.readAsBytes(); setModalState(() => templateBase64 = base64Encode(bytes)); _play('success'); _showOverlayToast('تم تحميل القالب بنجاح'); }
                    },
                ...
                // معاينة القالب المحسّنة (مطابقة للمستخدم)
                if (templateBase64 != null && templateBase64!.isNotEmpty)
                  Padding(
                    padding: const EdgeInsets.only(top: 12),
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      const Text('معاينة القالب:', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.grey)),
                            ...
                            child: Container(
                              ...
                              child: ClipRRect(
                                ...
                                child: Stack(children: [
                                  Positioned.fill(child: Image.memory(base64Decode(templateBase64!), fit: BoxFit.contain)),
                                  Positioned(
                                    left: (userViewX / 100) * 320,
                                    top: (userViewY / 100) * 200,
                                    child: Container(
                                      padding: const EdgeInsets.symmetric(horizontal: 4, vertical: 2),
                                      color: Colors.white.withOpacity(0.8),
                                      child: Text('12345678', style: TextStyle(fontSize: userViewFontSize * 0.7, fontWeight: FontWeight.bold, color: userViewColor)),
                                    ),
                                  ),
                                ]),
                              ),
                ...
                Row(children: [
                  ...
                  GestureDetector(onTap: () async { _play('click'); final color = await _openColorPicker(selectedColor); if (color != null) setModalState(() => selectedColor = color); }, child: Container(width: 40, height: 40, decoration: BoxDecoration(shape: BoxShape.circle, color: selectedColor, border: Border.all(color: Colors.grey)))),
                      ...
                      if (selectedNetworkId != null && newName.isNotEmpty && newPrice.isNotEmpty) {
                        ...
                        try {
                          var newCategory = {
                            ...
                            'userViewX': userViewX, 'userViewY': userViewY, 'userViewFontSize': userViewFontSize, 'userViewColor': userViewColor.value, 'allowSellSim': allowSellSim,
                          };
                          if (existingCat == null) {
                          ...
                          _play('success');
                          if (mounted) { Navigator.pop(ctx); _showOverlayToast('تم الحفظ بنجاح! 📋'); }
                        } catch (e) { setModalState(() => isSubmitting = false); _play('error'); }
                      } else { _play('error'); _showOverlayToast('الرجاء تعبئة الحقول الأساسية!', isError: true); }
                    },
                ...
                builder: (context, snapshot) {
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      var card = cards[index]; String pin = card['pin'];
                      return Card(
                        ...
                        child: ListTile(
                          ...
                          trailing: Row(mainAxisSize: MainAxisSize.min, children: [
                            IconButton(icon: const Icon(Icons.copy, color: Colors.blue), tooltip: 'نسخ الكرت', onPressed: () { _play('click'); Clipboard.setData(ClipboardData(text: pin)); _showOverlayToast('تم نسخ رقم الكرت للحافظة'); }),
                            IconButton(icon: const Icon(Icons.archive, color: Colors.orange), tooltip: 'نقل للأرشيف', onPressed: () async {
                              if (await _confirmAction("أرشفة الكرت", "سيتم سحب هذا الكرت من السوق ونقله للأرشيف. هل أنت متأكد؟", Colors.orange)) {
                                ...
                                List cats = List.from(netDoc['categories']); int idx = cats.indexWhere((c) => c['id'] == catId);
                                if (idx != -1) { cats[idx]['realStock'] = (cats[idx]['realStock'] ?? 1) - 1; cats[idx]['stock'] = (cats[idx]['realStock'] ?? 0) + (cats[idx]['simStock'] ?? 0); await _db.collection('networks').doc(netId).update({'categories': cats}); }
                                _showOverlayToast('تمت أرشفة الكرت بنجاح');
        ...
        builder: (ctx, setModalState) => Directionality(
          ...
          child: AlertDialog(
            ...
            content: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              const SizedBox(height: 15),
              SwitchListTile(title: const Text('حالة التوليد التلقائي', style: TextStyle(fontWeight: FontWeight.bold)), value: isBotEnabled, activeColor: Colors.purple, onChanged: (v) => setModalState(() => isBotEnabled = v)),
              if (isBotEnabled) ...[
                const SizedBox(height: 10),
                TextField(decoration: const InputDecoration(labelText: 'الحد الأدنى للمخزون (متى يبدأ البوت؟)', border: OutlineInputBorder()), keyboardType: TextInputType.number, onChanged: (v) => minStock = int.tryParse(v) ?? 5, controller: TextEditingController(text: minStock.toString())),
                const SizedBox(height: 10),
                TextField(decoration: const InputDecoration(labelText: 'كمية التوليد التلقائي (كم كرت ينتج؟)', border: OutlineInputBorder()), keyboardType: TextInputType.number, onChanged: (v) => refillAmount = int.tryParse(v) ?? 50, controller: TextEditingController(text: refillAmount.toString())),
              ]
            ]),
            actions: [
              ...
              ElevatedButton(
                ...
                onPressed: () async {
                  ...
                  var netDoc = await _db.collection('networks').doc(netId).get();
                  List cats = List.from(netDoc['categories']); int idx = cats.indexWhere((c) => c['id'] == catId);
                  cats[idx]['botMinStock'] = minStock; cats[idx]['botRefillAmount'] = refillAmount; cats[idx]['isBotEnabled'] = isBotEnabled;
                  await _db.collection('networks').doc(netId).update({'categories': cats});
                  Navigator.pop(ctx);
                  _play('success');
                  _showOverlayToast('تم حفظ إعدادات البوت الذكي');
                },
  ...
  Future<void> _deleteNetworkAndArchiveCards(SystemProvider sys, String networkId) async {
    ...
    try {
      ...
      for (var doc in cardsSnap.docs) { batch.update(doc.reference, {'status': 'archived'}); }
    ...
    } catch (e) { _play('error'); _showOverlayToast('فشل في حذف الشبكة: $e', isError: true); }
    finally { setState(() => _isProcessing = false); }
  }

  Future<void> _deleteCategoryAndArchive(String netId, Map<String, dynamic> category) async {
    bool confirm = await _confirmAction("حذف الفئة ونقل كروتها للأرشيف", "سيتم أرشفة جميع كروت هذه الفئة ثم حذف الفئة. متأكد؟", Colors.red, requirePassword: true);
    if (!confirm) return;
    _play('click');
    setState(() => _isProcessing = true);
    try {
      ...
      final batch = _db.batch();
      var cardsQuery = _db.collection('cards').where('categoryId', isEqualTo: catId).where('status', whereIn: ['متاح', 'print_ready']);
      final cardsSnap = await cardsQuery.get();
      for (var doc in cardsSnap.docs) { batch.update(doc.reference, {'status': 'archived'}); }
      final netRef = _db.collection('networks').doc(netId);
      batch.update(netRef, {'categories': FieldValue.arrayRemove([category])});
      await batch.commit();
      _play('success');
      _showOverlayToast('تم أرشفة كروت الفئة وحذفها');
      _logAction('delete_category', cardsSnap.docs.length, details: catId);
    } catch (e) { _play('error'); _showOverlayToast('فشل في حذف الفئة: $e', isError: true); }
    finally { setState(() => _isProcessing = false); }
  }
  ...
  List<Map<String, dynamic>> _collectOrders() {
    ...
    _multiGenControllers.forEach((key, controller) {
      if (controller.text.isNotEmpty) { int amt = int.tryParse(controller.text) ?? 0; if (amt > 0) { var parts = key.split('_'); orders.add({"networkId": parts[0], "categoryId": parts[1], "amount": amt}); } }
    });
    return orders;
  }
  ...
  Future<void> _simulateGenerate(String networkId, String categoryId, int amount) async {
    ...
    for (int i = 0; i < amount; i++) {
      ...
      batch.set(_db.collection('cards').doc(), {
        'categoryId': categoryId, 'networkId': networkId, 'pin': pin, 'status': 'متاح', 'isSimulated': true,
        'createdAt': FieldValue.serverTimestamp(), 'agentPhone': Provider.of<SystemProvider>(context, listen: false).currentUserPhone,
      });
    }
    final netDoc = await _db.collection('networks').doc(networkId).get();
    List cats = List.from((netDoc.data() as Map)['categories']); int idx = cats.indexWhere((c) => c['id'] == categoryId);
    if (idx != -1) { cats[idx]['simStock'] = (cats[idx]['simStock'] ?? 0) + amount; cats[idx]['stock'] = (cats[idx]['realStock'] ?? 0) + (cats[idx]['simStock'] ?? 0); batch.update(_db.collection('networks').doc(networkId), {'categories': cats}); }
    await batch.commit();
  }

  Future<void> _realGenerate(String networkId, String categoryId, int amount) async {
    final sys = Provider.of<SystemProvider>(context, listen: false);
    final response = await http.post(Uri.parse("$_renderUrl/generateMikrotikCards"), headers: {"Content-Type": "application/json"}, body: jsonEncode({
      "networkId": networkId, "categoryId": categoryId, "amount": amount, "agentPhone": sys.currentUserPhone, "forPrint": false,
    }));
    if (response.statusCode == 200) {
      final netDoc = await _db.collection('networks').doc(networkId).get();
      List cats = List.from((netDoc.data() as Map)['categories']); int idx = cats.indexWhere((c) => c['id'] == categoryId);
      if (idx != -1) { cats[idx]['realStock'] = (cats[idx]['realStock'] ?? 0) + amount; cats[idx]['stock'] = (cats[idx]['realStock'] ?? 0) + (cats[idx]['simStock'] ?? 0); await _db.collection('networks').doc(networkId).update({'categories': cats}); }
    } else { throw Exception('فشل توليد الكروت'); }
  }

  Future<bool> _startGeneration(SystemProvider sys, {required List<Map<String, dynamic>> orders}) async {
    if (orders.isEmpty) { _play('error'); _showOverlayToast('الرجاء كتابة كمية واحدة على الأقل', isError: true); return false; }
    int totalAmount = orders.fold(0, (sum, item) => sum + (item['amount'] as int));
    if (totalAmount > 400) { _play('error'); _showOverlayToast('الحد الأقصى للتوليد هو 400 كرت إجمالاً', isError: true); return false; }
    bool confirm = await _confirmAction("تأكيد التوليد المتعدد", "سيتم الآن توليد إجمالي $totalAmount كرت للمخزون. هل تريد الاستمرار؟", Colors.green);
    ...
    try {
      for (var order in orders) {
        if (_simulationMode) { await _simulateGenerate(order['networkId'], order['categoryId'], order['amount']); }
        else { await _realGenerate(order['networkId'], order['categoryId'], order['amount']); }
      }
      _play('success'); _multiGenControllers.forEach((k, v) => v.clear());
    ...
    } catch (e) { _play('error'); _showOverlayToast('فشل التوليد: $e', isError: true); success = false; }
    finally { setState(() => _isProcessing = false); }
    return success;
  }
  ...
  Future<void> _testConnection(Map<String, dynamic> net) async {
    ...
    try {
      final response = await http.post(Uri.parse("$_renderUrl/testConnection"), headers: {"Content-Type": "application/json"}, body: jsonEncode({
        "host": net['ip'], "user": net['apiUser'], "pass": net['apiPassword'], "port": net['apiPort']
      }));
      if (response.statusCode == 200) { _play('success'); _showOverlayToast('تم الاتصال بالميكروتك بنجاح! ✅'); }
      else { throw 'الراوتر لا يستجيب'; }
    } catch (e) { _play('error'); _showOverlayToast('خطأ في الاتصال: $e ❌', isError: true); }
...
class ServersTab extends StatefulWidget {
  ...
  @override State<ServersTab> createState() => _ServersTabState();
}

class _ServersTabState extends State<ServersTab> with AutomaticKeepAliveClientMixin {
  ...
  @override Widget build(BuildContext context) {
    ...
    return StreamBuilder<QuerySnapshot>(
      stream: p._db.collection('networks').where('agentPhone', isEqualTo: sys.currentUserPhone).snapshots(),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) return const Center(child: CircularProgressIndicator());
        final networks = snapshot.hasData ? snapshot.data!.docs : <QueryDocumentSnapshot>[];
        if (networks.isEmpty) return const Center(child: Text('لم تقم بربط أي شبكة ميكروتك حتى الآن.', style: TextStyle(color: Colors.grey)));
              ...
              return Card(
                ...
                child: Padding(
                  ...
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    const SizedBox(height: 12),
                    _infoRow(Icons.location_on, 'الموقع', net['location'] ?? '', textColor),
                    const SizedBox(height: 6),
                    _infoRow(Icons.wifi, 'IP', net['ip'] ?? '', textColor),
                    const SizedBox(height: 6),
                    _infoRow(Icons.info_outline, 'الحالة', isActive ? 'نشط' : 'مجمد', textColor),
                    const SizedBox(height: 16),
                    Wrap(spacing: 8, runSpacing: 8, children: [
                      _actionBtn(Icons.bolt, 'اختبار', Colors.blue, () => p._testConnection(net)),
                      _actionBtn(isActive ? Icons.pause_circle_filled : Icons.play_circle_fill, isActive ? 'تجميد' : 'تنشيط', Colors.orange, () async {
                        bool confirm = await p._confirmAction(isActive ? "تجميد الشبكة" : "تنشيط الشبكة", "هل تريد تغيير حالة الشبكة؟", Colors.orange);
                        if (confirm) {
                          p._play('click');
                          await p._db.collection('networks').doc(networks[index].id).update({'isActive': !isActive});
                          final netDoc = await p._db.collection('networks').doc(networks[index].id).get();
                          List cats = List.from((netDoc.data() as Map)['categories'] ?? []);
                          for (int i = 0; i < cats.length; i++) { cats[i]['isActive'] = !isActive; }
                          await p._db.collection('networks').doc(networks[index].id).update({'categories': cats});
                          p._play('success');
                          p._showOverlayToast(isActive ? 'تم تجميد الشبكة وفئاتها' : 'تم تنشيط الشبكة وفئاتها');
                        }
                      }),
                      _actionBtn(Icons.edit, 'تعديل', Colors.grey, () => p._showAddServerBottomSheet(sys, existingData: net, docId: networks[index].id)),
                      _actionBtn(Icons.delete, 'حذف', Colors.red, () => p._deleteNetworkAndArchiveCards(sys, networks[index].id)),
                    ]),
                  ]),
                ),
              );
            }),
          floatingActionButton: FloatingActionButton.extended(onPressed: () => p._showAddServerBottomSheet(sys), backgroundColor: Colors.blue.shade800, icon: const Icon(Icons.add, color: Colors.white), label: const Text('إضافة سيرفر', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold))),
        );
      },
    );
  }

  Widget _infoRow(IconData icon, String label, String value, Color textColor) => Row(children: [Icon(icon, size: 16, color: Colors.grey), const SizedBox(width: 8), Text('$label: ', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: Colors.grey)), Expanded(child: Text(value, style: TextStyle(fontSize: 13, color: textColor)))]);
...
class CategoriesTab extends StatefulWidget {
  ...
  @override State<CategoriesTab> createState() => _CategoriesTabState();
}
  ...
  @override Widget build(BuildContext context) {
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) return const Center(child: CircularProgressIndicator());
        final networks = snapshot.hasData ? snapshot.data!.docs : <QueryDocumentSnapshot>[];
        if (networks.isEmpty) return const Center(child: Text('أضف سيرفر شبكة أولاً لعرض فئاته.', style: TextStyle(color: Colors.grey)));
        final textColor = Theme.of(context).textTheme.bodyMedium?.color ?? Colors.black87;
        return Scaffold(
          backgroundColor: Colors.transparent,
          body: ListView.builder(
            ...
            itemBuilder: (context, netIndex) {
              ...
              return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                ...categories.map((category) {
                  int realStock = category['realStock'] ?? 0; int simStock = category['simStock'] ?? 0; int totalStock = realStock + simStock; bool isLowStock = totalStock < 10;
                  Color catColor = Color(category['color'] ?? Colors.blue.value); bool isCatActive = category['isActive'] ?? true; bool allowSellSim = category['allowSellSim'] ?? false;
                  return Card(
                      ...
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                          ...
                          Wrap(spacing: 4, children: [
                            IconButton(icon: const Icon(Icons.remove_red_eye, color: Colors.teal, size: 20), onPressed: () => p._showCardsList(netId, category['id'], category['name'], catColor), constraints: const BoxConstraints(), padding: const EdgeInsets.symmetric(horizontal: 5), tooltip: 'عرض الكروت'),
                            IconButton(icon: Icon(Icons.smart_toy, color: category['isBotEnabled'] == true ? Colors.purple : Colors.grey, size: 20), onPressed: () => p._showBotSettings(netId, category['id'], category), constraints: const BoxConstraints(), padding: const EdgeInsets.symmetric(horizontal: 5), tooltip: 'البوت الذكي'),
                            IconButton(icon: Icon(isCatActive ? Icons.pause_circle_filled : Icons.play_circle_fill, color: Colors.orange, size: 20), onPressed: () async {
                              bool confirm = await p._confirmAction(isCatActive ? "تجميد الفئة" : "تنشيط الفئة", "تغيير حالة الفئة؟", Colors.orange);
                              if (confirm) { p._play('click'); List updated = List.from(categories); int idx = updated.indexWhere((c) => c['id'] == category['id']); updated[idx]['isActive'] = !isCatActive; await p._db.collection('networks').doc(netId).update({'categories': updated}); p._play('success'); p._showOverlayToast(isCatActive ? 'تم تجميد الفئة' : 'تم تنشيط الفئة'); }
                            }, constraints: const BoxConstraints(), padding: const EdgeInsets.symmetric(horizontal: 5)),
                            IconButton(icon: const Icon(Icons.edit, color: Colors.blue, size: 20), onPressed: () => p._showAddCategoryBottomSheet(networks, existingCat: category, preSelectedNetId: netId), constraints: const BoxConstraints(), padding: const EdgeInsets.symmetric(horizontal: 5)),
                            IconButton(icon: const Icon(Icons.delete, color: Colors.red, size: 20), onPressed: () => p._deleteCategoryAndArchive(netId, category), constraints: const BoxConstraints(), padding: const EdgeInsets.symmetric(horizontal: 5)),
                          ]),
                        ]),
                        const SizedBox(height: 8),
                        ...
                        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                          ...
                          Row(children: [
                            Column(crossAxisAlignment: CrossAxisAlignment.end, children: [Text('حقيقي: $realStock', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Colors.green.shade800)), Text('وهمي: $simStock', style: TextStyle(fontSize: 12, color: Colors.orange.shade800))]),
                        ...
                        SwitchListTile(title: const Text('السماح ببيع الكروت الوهمية', style: TextStyle(fontSize: 13)), value: allowSellSim, activeColor: Colors.orange, dense: true, contentPadding: EdgeInsets.zero, onChanged: (v) async { p._play('click'); List updated = List.from(categories); int idx = updated.indexWhere((c) => c['id'] == category['id']); updated[idx]['allowSellSim'] = v; await p._db.collection('networks').doc(netId).update({'categories': updated}); }),
                      ]),
                    ),
                  );
                }).toList(),
          ...
          floatingActionButton: FloatingActionButton.extended(onPressed: () => p._showAddCategoryBottomSheet(networks), backgroundColor: Colors.orange.shade700, icon: const Icon(Icons.add_circle, color: Colors.white), label: const Text('إضافة فئة', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold))),
        );
      },
    );
  }
...
class GenerateTab extends StatefulWidget {
  ...
  @override State<GenerateTab> createState() => _GenerateTabState();
}

class _GenerateTabState extends State<GenerateTab> with AutomaticKeepAliveClientMixin {
  @override bool get wantKeepAlive => true;
  ...
  @override Widget build(BuildContext context) {
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        for (var net in networks) {
          String netId = net.id; List cats = (net.data() as Map)['categories'] ?? [];
          for (var cat in cats) { if ((net.data() as Map)['isActive'] != false && cat['isActive'] != false) { allCategories.add({'networkId': netId, 'networkName': (net.data() as Map)['name'], 'category': cat}); } }
        }
          ...
          body: Column(children: [
            ...
            Expanded(
              child: ListView.builder(
                padding: const EdgeInsets.symmetric(horizontal: 16), itemCount: allCategories.length,
                itemBuilder: (context, index) {
                  var item = allCategories[index]; String key = "${item['networkId']}_${item['category']['id']}"; p._multiGenControllers.putIfAbsent(key, () => TextEditingController());
                  return Card(
                    color: Theme.of(context).cardColor, elevation: 1, margin: const EdgeInsets.only(bottom: 10),
                    child: Padding(
                      padding: const EdgeInsets.all(12.0),
                      child: Row(children: [
                        Icon(Icons.category, color: Color(item['category']['color']), size: 30), const SizedBox(width: 15),
            ...
            Container(
              padding: const EdgeInsets.all(16),
              child: Row(children: [
                Expanded(child: SizedBox(height: 55, child: ElevatedButton.icon(onPressed: p._isProcessing ? null : () => p._startGeneration(sys, orders: p._collectOrders()), icon: const Icon(Icons.inventory, color: Colors.white), label: const Text('توليد للمخزون', style: TextStyle(fontSize: 14, color: Colors.white, fontWeight: FontWeight.bold)), style: ElevatedButton.styleFrom(backgroundColor: Colors.blue, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)))))),
                const SizedBox(width: 10),
                Expanded(child: SizedBox(height: 55, child: ElevatedButton.icon(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const PrintScreen())), icon: const Icon(Icons.print, color: Colors.white), label: const Text('الذهاب للطباعة', style: TextStyle(fontSize: 14, color: Colors.white, fontWeight: FontWeight.bold)), style: ElevatedButton.styleFrom(backgroundColor: Colors.teal, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)))))),
              ]),
            ),
          ]),
        );
