...
class _MikrotikCategoriesScreenState extends State<MikrotikCategoriesScreen>
              ...
              children: [
                ...
                Expanded(
                  child: TabBarView(
                    controller: _tabController,
                    children: [
                      ServersTab(parent: this, networks: agentNetworks),
                      CategoriesTab(parent: this, networks: agentNetworks),
                      GenerateTab(parent: this, networks: agentNetworks),
                    ],
                  ),
                ),
              ],
  ...
  // ======================== نافذة إضافة فئة (مع معاينة القالب) ========================
  void _showAddCategoryBottomSheet(List<QueryDocumentSnapshot> agentNetworks,
    ...
    double userViewFontSize =
        existingCat?['userViewFontSize']?.toDouble() ??
            _draftUserViewFontSize;
                      ...
                      child: TextField(
                          decoration: const InputDecoration(
                              ...
                              prefixIcon: Icon(Icons.data_usage,
                                  color: Colors.teal)),
                          controller:
                              TextEditingController(text: newCapacity),
      ...
      for (var order in orders) {
        if (_simulationMode) {
          await _simulateGenerate(order['networkId'],
              order['categoryId'], order['amount']);
        } else {
          await _realGenerate(order['networkId'],
              order['categoryId'], order['amount']);
        }
      }
      _play('success');
...
class ServersTab extends StatefulWidget {
  ...
  const ServersTab({required this.parent, required this.networks, super.key});
    ...
    return Scaffold(
      ...
      body: widget.networks.isEmpty
          ...
          : ListView.builder(
              ...
              itemBuilder: (context, index) {
                var net = widget.networks[index].data()
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () => p._showAddServerBottomSheet(sys),
        backgroundColor: Colors.blue.shade800,
...
class CategoriesTab extends StatefulWidget {
  final _MikrotikCategoriesScreenState parent;
  final List<QueryDocumentSnapshot> networks;
  const CategoriesTab({required this.parent, required this.networks, super.key});
...
class _CategoriesTabState extends State<CategoriesTab>
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: widget.networks.isEmpty
          ...
          : ListView.builder(
              ...
              itemBuilder: (context, netIndex) {
                var netId = widget.networks[netIndex].id;
                var netData = widget.networks[netIndex].data()
                    as Map<String, dynamic>;
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () => p._showAddCategoryBottomSheet(widget.networks),
...
class GenerateTab extends StatefulWidget {
  final _MikrotikCategoriesScreenState parent;
  final List<QueryDocumentSnapshot> networks;
  const GenerateTab({required this.parent, required this.networks, super.key});
...
class _GenerateTabState extends State<GenerateTab>
  ...
  Widget build(BuildContext context) {
    ...
    for (var net in widget.networks) {
      String netId = net.id;
