...
class _NetworkStoreScreenState extends State<NetworkStoreScreen> {
                  ...
                  children: [
                    ...
                    if (!isPurchased) ...[
                                ...
                                : () async {
                                    ...
                                    try {
                                      ...
                                      for (int i = 0; i < quantity; i++) {
                                        String realPin = await systemProvider
                                            .executeRealPurchase(unitPrice,
                                                title, agentPhone, categoryId);
                                        pins.add(realPin);
                                      }
                                      actualPinFetched = pins.join(' , ');
                                      // إنقاص المخزون
                                      await _decrementStock(
                                          agentPhone, categoryId, quantity);
                                      if (appliedCouponDocId != null) {
                                        await _db
                                            .collection('coupons')
                                      ...
                                      updateState(() {
                                        ...
                                        if (displayData != null) {
                                          purchasedNetworkName =
                                              displayData['networkName'] ?? '';
                                          purchasedLoginUrl =
                                              displayData['loginUrl'] ?? '';
                                          purchasedNote =
                                              displayData['note'] ?? '';
                                          purchasedTime =
                                              displayData['time'] ?? '';
                      ...
                      if (isLoadingCardData)
                        ...
                        _buildSimpleSuccessView(
                            actualPinFetched, originalPrice)
                      else
  ...
  Future<void> _decrementStock(
      String agentPhone, String categoryId, int amount) async {
  ...
  Widget _buildAdvancedSuccessView(
    ...
    return Column(
      ...
      children: [
                ...
                child: LayoutBuilder(
                  builder: (context, constraints) {
                    ...
                    if (imageWidth != null &&
                        ...
                        imageWidth > 0 &&
                        imageHeight > 0) {
                      scale = (containerWidth / imageWidth).clamp(
                          0.0, containerHeight / imageHeight);
                      imageDisplayWidth = imageWidth * scale;
                      imageDisplayHeight = imageHeight * scale;
                    }
                    double offsetX =
                        (containerWidth - imageDisplayWidth) / 2;
                    double offsetY =
                        (containerHeight - imageDisplayHeight) / 2;
                    ...
                    return Stack(
                      children: [
                        ...
                        Positioned(
                          left: offsetX +
                              (userViewX / 100) * imageDisplayWidth,
                          top: offsetY +
                              (userViewY / 100) * imageDisplayHeight,
                          child: Text(
                            pin,
                            style: TextStyle(
                              fontSize: fontSize,
        ...
        _buildInfoCard(
            theme, Icons.wifi, 'الشبكة', networkName ?? 'غير معروف'),
        ...
        _buildInfoCard(
            ...
            'صالح حتى',
            _formatDateShort(DateTime.now().add(Duration(
                hours: int.tryParse(time?.replaceAll(RegExp(r'[^0-9]'), '') ??
                        '24') ??
                    24)))),
        ...
        Row(
          ...
          children: [
            ...
            if (!kIsWeb) ...[
              _actionChip(
                  Icons.share, 'مشاركة', () => _shareCard()),
              const SizedBox(width: 10),
              _actionChip(
                  Icons.save_alt, 'حفظ', () => _saveCardImage()),
              const SizedBox(width: 10),
            ],
  ...
  Future<void> _saveCardImage() async {
    ...
    try {
      ...
      final tempDir = await getTemporaryDirectory();
      final file = File(
          '${tempDir.path}/card_${DateTime.now().millisecondsSinceEpoch}.png');
      await file.writeAsBytes(byteData.buffer.asUint8List());
        ...
        for (var cat in categories) {
          if (cat['name'] == categoryName) {
            ...
            return {
              ...
              'userViewColor':
                  cat['userViewColor'] ?? Colors.black.value,
              'imageWidth': imgWidth,
                    ...
                    StreamBuilder<QuerySnapshot>(
                      ...
                      builder: (context, snapshot) {
                        ...
                        if (isPos && posAgents.isEmpty)
                          return const Center(
                              child: Text(
                                  'لم يتم ربطك بأي وكيل حتى الآن.',
                                  style: TextStyle(color: Colors.grey)));
                        if (!snapshot.hasData ||
                            snapshot.data!.docs.isEmpty)
                          return const Center(
                              child: Text(
                                  'لا توجد شبكات معروضة حالياً.',
                                  style: TextStyle(color: Colors.grey)));
                        ...
                        var networks = snapshot.data!.docs.where((doc) {
                          ...
                          if (!match && _searchQuery.isNotEmpty) {
                            final cats = List<Map<String, dynamic>>.from(
                                net['categories'] ?? []);
                            match = cats.any((cat) =>
                                (cat['name'] ?? '').toString().toLowerCase().contains(_searchQuery));
                          }
                          return match;
                        }).toList();
                    ...
                    StreamBuilder<QuerySnapshot>(
                      ...
                      builder: (context, snapshot) {
                        ...
                        if (!snapshot.hasData ||
                          ...
                          return const Center(
                              child: Text(
                                  'لا توجد نقاط بيع معروضة حالياً.',
                                  style: TextStyle(color: Colors.grey)));
  ...
  Widget _buildNetworkCard(Map<String, dynamic> network, bool isPos,
    ...
    if (isPos) {
      ...
      categories = categories
          .where((cat) =>
              allowedCatsForThisAgent.contains(cat['id']))
          .toList();
    }

    categories = categories
        .where((cat) => (cat['isActive'] ?? true) == true)
        .toList();
              ...
              children: [
                ...
                ...categories.map((cat) {
                  ...
                  if (currentQty > stock) {
                    currentQty = stock;
                    qtyCtrl.text = '$stock';
                    _qtyValues[key] = stock;
                  }

                  Uint8List? thumbBytes;
                  if (cat['templateBase64'] != null &&
                      cat['templateBase64'].isNotEmpty) {
                    try {
                      thumbBytes =
  ...
  Widget _buildPoSCard(
      Map<String, dynamic> pos, bool isCurrentPos) {
    List stock =
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                if (stock.isEmpty)
                  const Text(
                      'لم يقم الوكيل بإضافة كروت لهذه النقطة بعد.',
                      style: TextStyle(
