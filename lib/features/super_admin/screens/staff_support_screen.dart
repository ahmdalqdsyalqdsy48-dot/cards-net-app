...
class _StaffSupportScreenState extends State<StaffSupportScreen> with SingleTickerProviderStateMixin {
  ...
  // ==========================================
  // 1. نافذة إضافة موظف جديد 👥 (حقيقية)
  // ==========================================
  void _showAddStaffDialog(SystemProvider provider) {
              ...
              content: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    const Divider(),
                    const Text('الصلاحيات الممنوحة:', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.blueGrey)),
                ...
                ElevatedButton(
                  onPressed: isLoading ? null : () async {
                    if (nameController.text.isNotEmpty && phoneController.text.isNotEmpty && passwordController.text.isNotEmpty) {
                      ...
                      try {
                        // 1. إنشاء حساب الموظف في Firestore
                        await _db.collection('users').doc(phoneController.text.trim()).set({
                          ...
                          'role': 'staff',
                          'salary': salaryController.text.trim().isNotEmpty ? salaryController.text.trim() : '0',
                          'status': 'نشط',
                          'permissions': permissions, // 👈 حفظ خريطة الصلاحيات
                          'createdAt': FieldValue.serverTimestamp(),
                        });

                        if (mounted) {
                          Navigator.pop(context);
                          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تمت إضافة الموظف بنجاح ✅', textDirection: TextDirection.rtl), backgroundColor: Colors.green));
                        }
                      } catch (e) {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('يرجى ملء البيانات الأساسية! ❌', textDirection: TextDirection.rtl), backgroundColor: Colors.red));
                    }
                  },
                  child: isLoading 
  ...
  // ==========================================
  void _toggleStaffStatus(Map<String, dynamic> emp, SystemProvider provider) async {
    try {
  ...
  void _deleteStaff(Map<String, dynamic> emp, SystemProvider provider) {
    showDialog(
      context: context,
      builder: (context) => Directionality(
        ...
        child: AlertDialog(
          ...
          content: const Text('هل أنت متأكد من حذف هذا الموظف نهائياً؟'),
          actions: [
  ...
  void _paySalary(Map<String, dynamic> emp, SystemProvider provider) {
    final amountController = TextEditingController(text: emp['salary'].toString());
          ...
          return Directionality(
            ...
            child: AlertDialog(
              ...
              content: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Text('هل تقر بتسليم الراتب للموظف "${emp['name']}"؟ سيتم تسجيله كمصروفات.'),
                  const SizedBox(height: 10),
                  _buildTextField('مبلغ الراتب الفعلي (بالريال)', Icons.money, controller: amountController, isNumber: true),
                ],
              ),
              actions: [
                if (!isPaying)
                ...
                ElevatedButton(
                  ...
                  onPressed: isPaying ? null : () async {
                    ...
                    try {
                      ...
                      if (mounted) {
                        Navigator.pop(context);
                        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم تسليم الراتب وتسجيله بنجاح ✅', textDirection: TextDirection.rtl), backgroundColor: Colors.green));
                      }
                    } catch (e) {
                  ...
                  child: isPaying ? const SizedBox(width: 15, height: 15, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2)) : const Text('نعم، أقر بالتسليم', style: TextStyle(color: Colors.white)),
                ),
              ],
            ),
          );
  ...
  void _showTicketChat(Map<String, dynamic> ticket, String docId) {
    ...
    showDialog(
      ...
      builder: (context) => Directionality(
        textDirection: TextDirection.rtl,
        child: AlertDialog(
          title: Text('تذكرة ${ticket['id'] ?? docId} - ${ticket['agentName'] ?? 'مجهول'}', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
          content: SizedBox(
            ...
            child: Column(
              ...
              children: [
                ...
                // عرض الردود السابقة والملاحظات (إذا وجدت)
  ...
  // ==========================================
  // واجهة التبويب الأول (جلب الموظفين من السيرفر)
  // ==========================================
  Widget _buildStaffTab(SystemProvider provider) {
    return Column(
      children: [
        Padding(
                      ...
                      child: Column(
                        children: [
                          Row(
                            ...
                            children: [
                              Text('${emp['name']} (موظف)', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16, decoration: isActive ? TextDecoration.none : TextDecoration.lineThrough)),
                          ...
                          Row(
                            ...
                            children: [
                              ...
                              Text('الراتب: ${emp['salary']} ريال', style: const TextStyle(color: Colors.grey, fontSize: 13)),
                            ],
                          ),
                          const Divider(),
                          Row(
                            ...
                            children: [
                              _buildIconButton(isActive ? Icons.pause_circle : Icons.play_circle, isActive ? 'إيقاف' : 'تفعيل', isActive ? Colors.orange : Colors.green, () => _toggleStaffStatus(emp, provider)),
                              _buildIconButton(Icons.delete, 'حذف', Colors.red, () => _deleteStaff(emp, provider)),
                              _buildIconButton(Icons.monetization_on, 'تسليم الراتب', Colors.green, () => _paySalary(emp, provider)),
                            ],
                          ),
                        ],
                      ),
  ...
  // ==========================================
  // واجهة التبويب الثاني (جلب التذاكر من السيرفر)
  // ==========================================
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Text('${ticket['id'] ?? 'بدون رقم'} | ${ticket['agentName'] ?? 'مجهول'}', style: TextStyle(fontWeight: FontWeight.bold, color: isClosed ? Colors.grey : null)),
