...
class _FinancialCenterScreenState extends State<FinancialCenterScreen>
          ...
          return Directionality(
            ...
            child: AlertDialog(
              ...
              content: Column(
                ...
                children: [
                  Text(
                      'يرجى كتابة سبب الرفض (سيصل للوكيل كإشعار):',
                      style: TextStyle(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildReceiptRow(
                      ...
                      isBold: true),
                  _buildReceiptRow(
                      'الطرف الآخر', log['agentName'] ?? 'مجهول'),
          ...
          return Directionality(
            ...
            child: AlertDialog(
              ...
              content: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 10),
                    _buildTextField('المبلغ (بالريال)',
                        Icons.money,
                        controller: amountController,
                        isNumber: true),
                    const SizedBox(height: 10),
                    _buildTextField('السبب (إجباري للسجل)',
                        Icons.edit_note,
                        controller: reasonController),
                  ],
                ),
              ...
              actions: [
                ...
                ElevatedButton(
                  style: ElevatedButton.styleFrom(
                      backgroundColor: settlementType == 1
                          ? Colors.green
                          : Colors.red,
                      shape: RoundedRectangleBorder(
                          borderRadius:
  ...
  void _showDangerLimitDialog(
    ...
    final limitController = TextEditingController(
        text: (agent['dangerLimit'] ?? 0).toString());
    showDialog(
      context: context,
        ...
        child: AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              const SizedBox(height: 15),
              _buildTextField(
                  'رصيد التنبيه (بالريال)', Icons.warning_amber,
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                try {
                  await wallet.updateDangerLimit(agent['phone'],
  ...
  // ==========================================
  void _showPdfStatementDialog(
      Map<String, dynamic> agent, WalletProvider wallet, SettingsProvider settings) {
    _play('click');
    DateTime? startDate;
    DateTime? endDate;
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setStateDialog) => Directionality(
          ...
          child: AlertDialog(
            ...
            shape: RoundedRectangleBorder(
                borderRadius: BorderRadius.circular(20)),
            title: Text('كشف حساب: ${agent['name']}',
                style: const TextStyle(fontWeight: FontWeight.bold)),
            content: SingleChildScrollView(
              child: Column(
                mainAxisSize: MainAxisSize.min,
              ...
              ElevatedButton.icon(
                ...
                onPressed: () async {
                  if (startDate == null || endDate == null) {
                    _play('error');
                    _showSnack('يرجى تحديد فترة الكشف!',
                        isErr: true);
                    return;
                  }
                  _play('click');
                  _showSnack('جاري توليد الـ PDF... ⏳');
                  await _generateAndDownloadPdf(agent, startDate!,
                      endDate!, wallet.transactionsLedger, settings.appName);
  ...
  Future<void> _generateAndDownloadPdf(Map<String, dynamic> agent,
      DateTime start, DateTime end, List<Map<String, dynamic>> allLedger, String appName) async {
    final endInclusive =
    ...
    pdf.addPage(
      pw.Page(
        ...
        theme: pw.ThemeData.withFont(
            base: arabicFont, bold: arabicBold),
            ...
            children: [
              pw.Container(
                ...
                child: pw.Row(
                  ...
                  children: [
                    pw.Text('كشف حساب وكيل',
                        style: pw.TextStyle(
                            color: PdfColors.white,
                            fontSize: 20,
                            fontWeight: pw.FontWeight.bold)),
                    pw.Text('نظام $appName',
                        style: pw.TextStyle(
                            color: PdfColors.white,
                            fontSize: 14)),
                  ],
              ...
              pw.Text(
                  'الفترة: من ${intl.DateFormat('yyyy-MM-dd').format(start)} إلى ${intl.DateFormat('yyyy-MM-dd').format(end)}',
    ...
    await Printing.sharePdf(
        ...
        filename: 'statement_${agent['phone']}.pdf');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: CustomHeader(
          title: 'المركز المالي (SaaS) - ${settings.appName}'),
      drawer: CustomDrawer(
                  ...
                  Tab(
                    child: Row(
                      ...
                      children: [
                        ...
                        if (wallet.pendingRechargeRequests
                            .isNotEmpty) ...[
                          const SizedBox(width: 4),
                          Container(
              ...
              child: TabBarView(
                ...
                children: [
                  _buildRechargeRequestsTab(
                      wallet, cardColor, textColor),
                  _buildWalletsTab(
                      wallet, settings, cardColor, textColor),
                  _buildLedgerTab(
                      wallet, settings, cardColor, textColor),
                ],
              ),
  ...
  Widget _buildRechargeRequestsTab(
    ...
    final requests =
        wallet.pendingRechargeRequests.where((req) {
      ...
      return (req['userName']
              ...
              req['agentName']
                  ...
                  .contains(query) ??
              false) ||
          (req['userPhone']?.toString().contains(query) ??
              req['agentPhone']?.toString().contains(query) ??
              false) ||
          (req['reference']?.toString().contains(query) ??
              req['ref']?.toString().contains(query) ??
              false);
    }).toList();
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 10),
            const Text(
                'لا توجد طلبات شحن معلقة حالياً، عمل رائع!',
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final docId = req['docId'];
        final isProcessing =
            _processingRequests.contains(docId);

        bool isSaaS = req['type'] == 'saas_quota';
        double quotaAmount =
            double.tryParse(req['amount'].toString()) ??
                0;
        double feeAmount =
            double.tryParse(req['fee']?.toString() ?? '0') ??
                0;
        String agentName = req['userName'] ??
            req['agentName'] ??
            'مجهول';
        String agentPhone = req['userPhone'] ??
            req['agentPhone'] ??
            '';
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 10),
                _buildInfoRow('البنك المُحوّل إليه:',
                    req['bankName'] ?? 'غير محدد',
                    textColor: textColor),
                _buildInfoRow('مصدر التحويل:',
                    req['transferSource'] ?? 'غير محدد',
                    textColor: textColor),
                _buildInfoRow('رقم المرجع:',
                    req['reference'] ?? req['ref'] ?? 'لا يوجد',
                    textColor: textColor),
                const SizedBox(height: 15),
                OutlinedButton.icon(
                  onPressed: () {
                    _play('click');
                    if (req['hasReceipt'] == true &&
                        req['receiptBase64'] != null) {
                      showDialog(
                        context: context,
                        builder: (_) => Dialog(
                          backgroundColor:
                      ...
                      _showSnack(
                          'الوكيل لم يقم بإرفاق صورة السند ⏳',
                          isErr: true);
                    }
                  },
                  icon: Icon(Icons.image,
                      size: 18,
                      color: req['hasReceipt'] == true
                          ? Colors.green
                          : Colors.grey),
                  label: Text(
                      req['hasReceipt'] == true
                          ? 'عرض صورة السند المرفق 📸'
                          : 'لا يوجد سند مرفق',
                      style: TextStyle(
                          color: req['hasReceipt'] == true
                              ? Colors.green
  ...
  Widget _buildWalletsTab(WalletProvider wallet, SettingsProvider settings,
    ...
    final wallets = wallet.agentsList.where((agent) {
      ...
      return (agent['name']
                  ...
                  .contains(query) ??
              false) ||
          (agent['phone']?.toString().contains(query) ??
              false);
    }).toList();

    if (wallets.isEmpty)
      return const Center(
          child: Text('لا يوجد وكلاء مطابقين للبحث.'));
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final balance =
            double.parse(agent['balance'].toString());
        final dangerLimit = double.parse(
            (agent['dangerLimit'] ?? 0).toString());
            ...
            child: Column(
              children: [
                ...
                Row(
                  ...
                  children: [
                    _buildIconButton(
                        Icons.settings, 'تسوية', Colors.blue,
                        () => _showManualSettlementDialog(
                            agent, wallet),
                        textColor),
                    _buildIconButton(
                        Icons.tune, 'حد الخطر', Colors.orange,
                        () => _showDangerLimitDialog(
                            agent, wallet),
                        textColor),
                    _buildIconButton(
  ...
  Widget _buildLedgerTab(WalletProvider wallet, SettingsProvider settings,
      Color cardColor, Color textColor) {
    final ledger = wallet.transactionsLedger.where((log) {
      ...
      return (log['agentName']
          ...
          (log['agentPhone']
                  ...
                  .contains(query) ??
              false) ||
          (log['type']?.toString().contains(query) ??
              false) ||
          (log['title']?.toString().contains(query) ??
              false);
    }).toList();
    ...
    if (ledger.isEmpty)
      return const Center(
          child: Text(
              'السجل المالي فارغ أو لا توجد نتائج للبحث.'));
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final amount =
            double.parse(log['amount'].toString());
        final isPositive =
            log['type'] == 'deposit' || log['type'] == 'income';
        ...
        return Card(
          ...
          child: InkWell(
            onTap: () =>
                _showTransactionReceipt(log, settings),
            borderRadius: BorderRadius.circular(15),
            child: Padding(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                            '${log['agentName'] ?? 'مجهول'} (${log['agentPhone'] ?? ''})',
                            style: const TextStyle(
                                fontSize: 11,
                                color: Colors.blueGrey)),
                        if (log['reason'] != null)
                  ...
                  Column(
                    ...
                    children: [
                      Text(
                          '${isPositive ? '+' : ''}${intl.NumberFormat('#,###').format(amount)}',
                          style: TextStyle(
                              fontWeight:
                                  FontWeight.bold,
                              color: color,
  ...
  Widget _buildIconButton(IconData icon, String label,
      Color color, VoidCallback onTap, Color textColor) {
    return InkWell(
      onTap: onTap,
      borderRadius: BorderRadius.circular(15),
      child: Container(
